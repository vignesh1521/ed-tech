...
type AddCourseCardProps = {
    ...
    courses: Course_Type[];
    setCourses: React.Dispatch<React.SetStateAction<Course_Type[]>>;
};



export default function AddCourseCard({ courseCard, setCourses, courses }: AddCourseCardProps) {
    ...
    async function handleCourseAdd() {
        ...
        if (!description.trim()) {
            ...
            setError("Course description requires 50 to 150 characters");
            return;
        }
        ...
        setLoading(true)
        const mutation = `
mutation($courseTitle: String!, $imageUrl: String!, $description: String!, $price:String! ,$level: String!){
  addNewCourse(courseTitle: $courseTitle, imageUrl: $imageUrl, description: $description,price:$price,  level: $level) {
        id
  }
}
    `;
        ...
        try {
            ...
            if (result.errors) {
                ...
                courseCard(false)
                const newCourse: Course_Type = {
                    id: result.data.addNewCourse.id,
                    title,
                    description,
                    level,
                    image: imageUrl,
                ...
                <div className='coure_form'>
                    ...
                    <div className='course_card_actions'>
                        ...
                        <button type="submit" onClick={() => handleCourseAdd()}>
                            {
                                loading ? <div className="loader_25"></div> : "Add Course"
                            }
                        </button>
                    </div>
                </div>
